...
public class CommentRepository : ICommentRepository
{
    ...
    {
        return await _context.Comments.Where(c => c.ProductId == productId).Include(c => c.Upvotes).ToListAsync()
            .ConfigureAwait(false);
    }

    public async Task<List<Comment>> GetCommentsByUser(Guid userId)
    {
        return await _context.Comments.Where(c => c.UserId == userId).Include(c => c.Upvotes).ToListAsync()
            .ConfigureAwait(false);
    }

    public async Task<Comment?> GetCommentById(Guid id)
    {
        return await _context.Comments.Include(c => c.Upvotes).FirstOrDefaultAsync(c => c.Id == id)
            .ConfigureAwait(false);
    }
